...
Currently all programs are implemented in Python, and tested with basic unit tests and the Hyothesis testing framework. All source code samples are linted as well with ruff and mypy. Suffice it to say that testing has been essential in checking the correctness of the puzzle solutions collected in this work.
