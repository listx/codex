...
subarray, starting at that iteration's =x= as the initiail value (the =else=
block in the code above). This makes sense because if a subarray is zero or
negative in value, it is not going to help make any other subarray larger. We're
...
the solution to a smaller version of the problem to solve the bigger problem. It
