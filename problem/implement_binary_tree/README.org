...
[[file:~/prog/codex/problem/implement_binary_search_tree/README.html][Binary /search/ trees]], or BSTs, are binary trees with additional ordering
...
=nodes_at_current_depth= variable (it is no longer only holding nodes at the
current level, but instead at the current and next level as it gets mutated
in-place).
...
For making test data more uniform, we define =make_perfect_tree()= to create
perfect binary trees.  Perfect trees have nodes at every level. For example, a
perfect tree of depth 3 (which has \(2^3 - 1 = 7\) nodes) looks like this:
